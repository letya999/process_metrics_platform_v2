...
def test_flow_efficiency_ignores_intervals_after_first_completion():
    issues = pl.DataFrame({"id": ["i1"], "project_id": ["p1"], "key": ["P1-1"]})
    changelog = pl.DataFrame(
        {
            "issue_id": ["i1", "i1", "i1", "i1", "i1"],
            "to_status_id": ["todo", "in_progress", "done", "todo", "in_progress"],
            "changed_at": [
                datetime(2026, 1, 1, 0, 0),
                datetime(2026, 1, 3, 0, 0),
                datetime(2026, 1, 7, 0, 0),  # first completion
                datetime(2026, 1, 8, 0, 0),  # should be ignored
                datetime(2026, 1, 9, 0, 0),  # should be ignored
            ],
        }
    )
